...
export function AuthenticatedPage() {
  ...
  }, [documents.length, query]);

  const visibleDocuments = useMemo(() => {
    const filename = query.trim().toLocaleLowerCase();
    if (!filename) return documents;
    return documents.filter((document) => document.filename.toLocaleLowerCase().includes(filename));
  }, [documents, query]);
  ...
  const logout = async () => {
    ...
    try {
      ...
      window.dispatchEvent(new Event('docsense:logout'));
      goTo('/');
    } catch (cause) {
